...
export default function TaskDetails() {
    ...
    const fetchSuggestions = async () => {
      try {
        ...
        const result = await getAISuggestions({ 
          prompt: `Give me some suggestions for how to complete the following task: ${task.title}`
        });
  ...
  if (isFocusMode) {
    return <FocusMode task={task} onExit={() => setIsFocusMode(false)} />;
  }
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 space-y-4">
        <div className="flex items-center justify-between">
            ...
            <Button onClick={() => setIsFocusMode(true)}>Enter Focus Mode</Button>
        </div>
